# 使用 <span lang="ja">てください</span> 的柔性请求和命令形

<span lang="ja">て</span> 形也可以用于发出简单的命令或指令。当我们在 <span lang="ja">て</span> 形后面加上 <span lang="ja">ください</span> 时，我们是在礼貌地请求某人做某事（为我们），实际上是在给他们一个命令。这种用法非常常见，以至于 <span lang="ja">ください</span> 部分可以被完全省略，但语句仍然保有命令感。省略 <span lang="ja">ください</span> 使其更随意，可能听起来更直接（可能粗鲁）。
...
这引出了第四个常见的动词词干——命令形。这是一个"真正的"命令形，而不是仅仅使用 <span lang="ja">てください</span> 所发出的简单请求。请注意，命令形通常听起来很粗鲁。

一段动词的命令形用 <span lang="ja">ろ</span> 构成，五段动词的命令形通过将 u 音替换为 e 音来构成。
